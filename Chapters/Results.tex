...
An example of tracking an inconsistently tuned Kalman Filter can be seen in \Cref{fig:KFinconsistent}.
\begin{figure}[H]
	\centering
...
\begin{figure}[H]
	...
	\end{subfigure}\hfill
	\begin{subfigure}[b]{1.0\textwidth}
		\includegraphics[width=1.0\linewidth]{fusionCTRV}
...
\subsection{Fudging}
Should one or more of the consistency checks fail, then the designer of the estimator should suspect a modeling error. A step back to an earlier design stage has to be taken in order to identify the fault. The source of the problem is not a trivial process to locate. An inaccurate motion model, wrong process $\proccCov$ or observation $\measCov$ covariance sizing, higher order errors due to Taylor approximations (see \Cref{ssec:limitations} only to name a few. A heuristic method proposed by \cite{BarShalom1980} is to increase the modeled process noise $\proccCov$
...
In \Cref{fig:detp} one can see the effect of fusing asynchronous sensor observations on the determinant of the state covariance matrix $P_k$, whereas in \Cref{fig:dettracepk} one can see the difference between using the determinant or the trace of the state covariance matrix as an uncertainty metric.
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\linewidth]{zoomViewPF.pdf}
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\linewidth]{zoomViewEKF.pdf}
